<template>
  <v-main class="pa-0">
    <v-data-table
      ...
      :search="search"
      class="elevation-1"
      no-data-text="لا يوجد تصنيفات حتى الآن.."
    ...
    >
      <template v-slot:top>
        ...
        >
          ...
          <v-dialog v-model="dialogDelete" max-width="500px">
            <v-card>
              <v-card-title>هل انت متأكد انك تريد حذف هذا التصنيف؟</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-btn
          ...
          @click="getSub(item)"
        >
          <v-icon small>mdi-layers-search</v-icon>
        </v-btn>
        <v-btn
          icon
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return this.editedIndex === -1 ? 'اضف تصنيف جديد' : 'تعديل التصنيف'
    },
  },
  ...
  methods: {
    ...
    addCats () {
      if (this.$refs.cats.validate()) {
        ...
        this.$axios.$post('/api/Category/Create', catiegory)
          .then(() => {
            this.msgText = 'تمت إضافة التصنيف بنجاح.'
            this.message = true
            this.getCategories()
            this.close()
          })
    ...
    // Get Sub Categories
    getSub (item) {
      this.$axios.$get(`/api/Category/${item.id}/SubCategories`)
        .then(() => {
          console.log("DONE")
        })
        .catch((err) => {
          console.log(err)
          this.msgText = 'لا يوجد فئات فرعية.'
          this.info = true
        })
    },
    ...
    submitEdit () {
      if (this.$refs.cats.validate()) {
        ...
        this.$axios.$post('/api/Category/Update', catiegory)
          .then(() => {
            this.msgText = 'تم تعديل التصنيف بنجاح.'
            this.message = true
            this.getCategories()
            this.close()
          })
    ...
    // Delete Confirmation
    deleteItemConfirm () {
      this.$axios.$post('/api/Category/Delete', this.data[this.editedIndex].id)
      .then(() => {
        this.msgText = 'تم حذف التصنيف بنجاح.'
        this.message = true
